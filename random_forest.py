...
class RandomForest(object):
    ...
    def _bootstrapping(self, num_training, num_features, random_seed = None):
        ...
        ##################################################################################
        row_idx = np.random.randint(0, num_training, size=num_training)
        some_features = int(num_features * self.max_features)
        col_idx = np.random.choice(num_features, size=some_features, replace=False)
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_estimators):
            ...
            X2 = X2[:, col_idx]
            y2 = y[row_idx]
            self.decision_trees[i] = self.decision_trees[i].fit(X2, y2)
